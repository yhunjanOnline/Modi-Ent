...
---------------------------------------------------------------------------
  -- PACKAGE
  -- NAME: xxgpil_receipt_pkg
  -- DESC: PACKAGE TO PROCESS AR TRANSACTIONS
  -- Modified for Transaction status  03-Aug-2021  Sanjeev SR21270
  ---------------------------------------------------------------------------
      ...
      IS
         ...
         SELECT a.customer_id, a.receipt_id, a.gpil_book_bank,
          ...
          WHERE a.receipt_id = b.receipt_id
            AND a.status = 'Inprocess'-- 'Pending'
   ...
   BEGIN
      ...
      begin -------Added by Sanjeev on 03Aug2021 ot change the status Inprocess
      ...
      exception when others then
         DBMS_OUTPUT.put_line ('Status-inprocess not updated');
      end;
